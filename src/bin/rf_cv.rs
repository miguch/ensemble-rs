...
fn main() {
    ...
    let forest_config = vec![
        ...
        RandomForestConfig::NEstimators(150),
    ];

    let rf = RandomForest::from_configs(tree, forest_config);

    let folds = cross_validate::KFold::new(3);
    let results = folds.cross_validate(rf, &train_data, &label_data, numeric::r2_score);
    println!("{:?}", results);
}
